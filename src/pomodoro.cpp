...
namespace {
// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables) // Required for ncurses TUI state
...
int prompt_selection(const std::string& prompt, const std::vector<std::string>& options, bool allow_quit) {
    int choice = 0;
    int key_code = 0;
...
bool prompt_continue(const char* msg, const SessionTime& study, const SessionTime& brk, bool is_break) {
    clear();
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg) // ncurses API
    mvprintw(kBreakPromptRow, 2, "%s", msg);
    if (is_break) {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg) // ncurses API
        mvprintw(kBreakHelpRow, 2, "Break time: %02d:%02d", brk.minutes, brk.seconds);
    } else {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg) // ncurses API
        mvprintw(kBreakHelpRow, 2, "Study time: %02d:%02d", study.minutes, study.seconds);
    }
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg) // ncurses API
...
bool handle_session_transition(
    bool& on_break, SessionTime& current, TimerTickState& tick_state,
    const SessionTime& pomodoro, const SessionTime& brk,
    std::string& status) {
    if (!on_break) {
        on_break = true;
        current = brk;
        ...
        status = "Break Ready";
        if (!prompt_continue("Study session complete! Time for a break.", pomodoro, brk, true)) {
            return false;
        }
        status = "Break Running";
        running = true;
    } else {
        if (!prompt_continue("Break complete! Press any key to start a new study session.", pomodoro, brk, false)) {
            return false;
        }
        on_break = false;
        current = pomodoro;
...
void pomodoro_event_loop(const SessionTime& pomodoro, const SessionTime& brk) {
    ...
    TimerTickState tick_state{current.minutes * kSecondsPerMinute + current.seconds, 0};
    const int initial_total_seconds = tick_state.total_seconds;
    std::string status = "Stopped";
    bool on_break = false;
    draw(current.minutes, current.seconds, status, initial_total_seconds, tick_state.total_seconds);
    while (true) {
        int key_code = getch();
        if (key_code == 'q') { break; }
        if (key_code == 's') {
        ...
        if (running && !paused) {
            if (timer_tick(tick_state, kIntervalUs)) {
                running = false;
                if (!handle_session_transition(on_break, current, tick_state, pomodoro, brk, status)) {
                    break;
                }
            }
            int remaining_us = tick_state.total_seconds * kMicrosecondsPerSecond - tick_state.elapsed_us;
            ...
            int display_seconds = (display_total_us / kMicrosecondsPerSecond) % kSecondsPerMinute;
            draw(display_minutes, display_seconds, status, initial_total_seconds, tick_state.total_seconds);
...
void draw_menu(const std::string& prompt, const std::vector<std::string>& options, int choice, bool allow_quit) {
    clear();
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg) // ncurses API
    ...
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg) // ncurses API
    mvprintw(quit_row + kMenuHelpRowOffset, 2, "Use UP/DOWN to select, ENTER to confirm");
    refresh();
}
...
void draw(int minutes, int seconds, const std::string& status, int total_seconds, int remaining_seconds) {
  ...
  std::string bar = "[" + std::string(fill, '#') + std::string(kBarWidth - fill, ' ') + "]";
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg) // ncurses API
